...
/// Checks if the config file exists. If not, it runs the first-time setup.
pub fn ensure_config_exists() -> Result<()> {
    let config_path = ConfigData::path()?;
...
/// Creates a default config file and a new dedicated keypair for the user.
